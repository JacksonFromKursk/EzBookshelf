...
test('renders all the book information', async () => {
	...
	expect(
		screen.getByRole('button', {name: /Добавить в список/i}),
	).toBeInTheDocument()

	expect(
		screen.queryByRole('button', {name: /Удалить из списка/i}),
	).not.toBeInTheDocument()
	expect(
		screen.queryByRole('button', {name: /Пометить как прочитанный/i}),
	).not.toBeInTheDocument()
	expect(
		screen.queryByRole('button', {name: /Пометить как непрочитанную/i}),
	).not.toBeInTheDocument()
	expect(
...
test('can create a list item for the book', async () => {
	...
	const addToListButton = screen.getByRole('button', {name: /Добавить в список/i})
	...
	expect(
		screen.getByRole('button', {name: /Пометить как прочитанный/i}),
	).toBeInTheDocument()
	expect(
		screen.getByRole('button', {name: /Удалить из списка/i}),
	).toBeInTheDocument()
	...
	expect(
		screen.queryByRole('button', {name: /Добавить в список/i}),
	).not.toBeInTheDocument()
	expect(
		screen.queryByRole('button', {name: /Пометить как непрочитанную/i}),
	).not.toBeInTheDocument()
...
test('can mark a list item as read', async () => {
	...
	const markAsReadButton = screen.getByRole('button', {name: /Пометить как прочитанный/i})
	await userEvent.click(markAsReadButton)
	expect(markAsReadButton).toBeDisabled()

	await waitForLoadingToFinish()
	expect(
		screen.getByRole('button', {name: /Пометить как непрочитанную/i}),
	).toBeInTheDocument()
